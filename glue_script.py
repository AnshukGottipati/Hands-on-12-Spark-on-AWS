...
print(f"Writing date-wise review counts results to {s3_analytics_path}daily_review_counts/ ...")

reviews_per_day_frame = DynamicFrame.fromDF(
    df_reviews_per_day.repartition(1),
    glueContext,
    "reviews_per_day_df"
)
glueContext.write_dynamic_frame.from_options(
    frame=reviews_per_day_frame,
    connection_type="s3",
...
print(f"Writing top 5 most active customers to {s3_analytics_path}top_5_customers/ ...")

top_5_customers_frame = DynamicFrame.fromDF(
    df_top_5_customers.repartition(1),
    glueContext,
    "top_5_customers_df"
)
glueContext.write_dynamic_frame.from_options(
    frame=top_5_customers_frame,
    connection_type="s3",
...
df_rating_distributions = spark.sql("""
    SELECT
        rating,
...
print(f"Writing rating distribution to {s3_analytics_path}rating_distributions/ ...")

rating_distributions_frame = DynamicFrame.fromDF(
    df_rating_distributions.repartition(1),
    glueContext,
    "rating_distributions_df"
)
glueContext.write_dynamic_frame.from_options(
    frame=rating_distributions_frame,
    connection_type="s3",
    connection_options={"path": s3_analytics_path + "rating_distributions/"},
    format="parquet"
)
